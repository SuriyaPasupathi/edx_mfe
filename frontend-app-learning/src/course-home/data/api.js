...
export async function getProgressTabData(courseId, targetUserId) {
  ...
  try {
    ...
    // Safely handle cases where grading_policy or grade_range might be undefined
    if (data.grading_policy && data.grading_policy.grade_range) {
      camelCasedData.gradingPolicy.gradeRange = data.grading_policy.grade_range;
    } else {
      // Default to empty object if grading policy is not configured
...
/**
 * Save certificate to ICG Portal via FastAPI service
 * @param {Object} certificateData - Certificate data including username, email, courseId, etc.
 * @returns {Promise<Object>} Response from FastAPI service
 */
export async function saveCertificateToICGPortal(certificateData) {
  const payload = {
    ...
    certificatePdfUrl: certificateData.certificatePdfUrl,
    certificateUuid: certificateData.certificateUuid,
    completedDate: certificateData.completedDate,
    grade: certificateData.grade,
    mode: certificateData.mode,
    status: certificateData.status || 'downloadable',
  };

  // Call FastAPI service which will forward to ICG API
  const fastApiUrl = getConfig().FASTAPI_BASE_URL || 'http://localhost:8000';
  const url = `${fastApiUrl}/webhook/course-completed`;
  ...
  try {
    ...
    // FastAPI returns: { "status": "success", "message": "...", "icg_response": {...} }
    // Extract ICG response if available, otherwise use FastAPI response
    if (result.icg_response) {
      return result.icg_response;
    }
    
    // Return success response in format expected by frontend
    return {
      success: result.status === 'success',
      message: result.message || 'Certificate saved successfully',
    };
  } catch (error) {
    logInfo(`Error saving certificate to ICG Portal via FastAPI: ${error.message}`);
    throw error;
  }
}
