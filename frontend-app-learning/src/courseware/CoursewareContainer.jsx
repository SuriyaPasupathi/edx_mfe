...
class CoursewareContainer extends Component {
  ...
  componentDidUpdate() {
    ...
    // Only redirect if the sequence has units and no unitId is specified
    if (sequenceStatus === 'loaded' && sequence && (!sequence.unitIds || sequence.unitIds.length === 0) && routeUnitId) {
      // If sequence has no units but URL has a unitId, remove the unitId from URL
      const baseUrl = `/course/${courseId}/${sequenceId}`;
      const sequenceUrl = isPreview ? `/preview${baseUrl}` : baseUrl;
      navigate(sequenceUrl, { replace: true });
    } else {
