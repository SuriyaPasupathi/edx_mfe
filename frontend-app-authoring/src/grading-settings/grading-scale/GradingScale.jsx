  ...
  const addNewGradingSegment = () => {
    setGradingSegments((prevSegments) => {
      ...
      if (prevSegments.length >= 5) {
        ...
        // Safely handle case where secondSegment might be undefined
        if (!firstSegment) {
          // If no segments exist, create default segments
          return [
            { current: 100, previous: 0 },
            { current: 60, previous: 0 },
          ];
        }
        if (!secondSegment) {
          // If only one segment exists, calculate based on first segment
          const newCurrentValue = Math.ceil(firstSegment.current / 2);
          const newSegment = {
